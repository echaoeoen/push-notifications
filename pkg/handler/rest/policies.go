...
func GetPolicyTenant(m police.Manager, cn police.Configuration) echo.HandlerFunc {
	return func(c echo.Context) error {
		...
		var filter [][3]string
		filter = append(filter, [3]string{"tenant", "=", tenant})

		resource := c.QueryParam("resource")
		if resource != "" {
			filter = append(filter, [3]string{"resource", "=", resource})
		}
		subject := c.QueryParam("subject")
		if resource != "" {
			filter = append(filter, [3]string{"resource", "=", subject})
		}
		acls, err := m.FetchPolicy(c.Request().Context(), filter...)
		if err != nil {
			cn.Logger().Error(err)
			return responseError(c, err)
		}
